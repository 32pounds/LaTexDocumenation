...
\begin{section}{Map and Movement}
 ...
 \begin{chapter}{Project Changes}
  ...
  \begin{subsection}{Player Expectations}
   ...
   which we did not want to do. The ideas for the player were scratched do to the fact that we would rather create a better 
  ...
  \begin{subsection}{Advanced Gameplay}
   ...
   until we decided to reroute to multiplayer.  The gameplay is different for a single 
  ...
  \begin{subsection}{Multiplayer}
   ...
   homework 6.  We needed to change the way our code worked for us to be able to run multiplayer 
   so we decided to fall away from a heavy single player and work more into creating a basic 
   but multiplayer enabled game.  We needed to decide what to include 
   in and how to send our packets.  We chose to send the gamestate through a UDP packet at 40 packets
   per second to each client.
  \end{subsection}
 \end{chapter}
\end{document}
