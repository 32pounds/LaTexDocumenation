...
\begin{chapter}{Project History}
	...
	This is where we chose to diverge from a very developed story line and work more into networking.  
	...
	\begin{enumerate}
		\item Player design
		\item Advanced game play i.e quests, factions, inventory
		\item Multiplayer
	\end{enumerate}
	...
	\begin{subsection}{Advanced Game play}
		Game play is something where the idea seems so easy but the implementation 
		takes much more thought.  So much needed to be implemented before we started game play 
		that we never really got around to it.  For instance, our game was structured for single player 
		until we decided to reroute to multiplayer.  The game play was different for a single 
		player than it is for multiplayer in our game.  Before we can completely work on the 
		game play we wanted i.e quests, combat, factions, we needed inventory, player health, moving monsters,
		and also monsters that react to your movement which is more difficult than one would think. 
		We ended up with making a game play in which you run around killing bugs that have invaded your office
		space, some bugs attack you, others don't. 
	\end{subsection}
	...
	\begin{subsection}{Multiplayer}
		...
		in the experience of making a multiplayer game, and we though that we might be the only team to successfully achieve full multiplayer.  We set up a meeting where the details of multiplayer were hashed out, like the structure of our packets.  We chose to send the complete state of the game through a UDP packet at 40 packets
...
include the player, monster, communications, game state and map.
  ...
  \begin{section}{Player Use Cases}
    \begin{subsection}{Speak with NPC}
      \textbf{Goal}:
      Player wishes to speak with an NPC in order to gain information or receive 
      a quest.
      ...
      connected to use cases correlating to interacting with NPCs through 
      dialog.
    ...
    \begin{subsection}{Movement through Area}
      ...
      \textbf{Related Use Cases}:
      All over world interactions are extended by this use case. Talking to npcs, combat, and picking up items are examples.
    ...
    \begin{subsection}{Inventory Item Use}
      \textbf{Actors}:
      Player, Inventory, Usable Item

      \textbf{Preconditions}:
      \begin{enumerate}
      	\item Inventory belongs to Player.
      	\item Item is in Inventory.
      	\item Item is Usable from the Inventory
      ...
      \textbf{Related Use Cases}
      All Usable Item Uses extend this use case. Examples would be quaffing
      ...
      \begin{enumerate}
        ...
        Usable Item and updates the Item's state accordingly.
      \end{enumerate}
    \end{subsection}
    ...
    \begin{subsection}{Interact with a Coworker}
      ...
      \begin{enumerate}
        ...
        \item The System performs the interaction, gathering additional input from the
	      Player as necessary.
      \end{enumerate}
    \end{subsection}
    ...
    \begin{subsection}{World Entity Interaction}
      ...
      \textbf{Actors}:
      Player, Intractable Entity

      \textbf{Preconditions}:
      \begin{enumerate}
        \item Player is adjacent to the Intractable Entity.
        \item Player inputs the interact command, selecting the Intractable Entity.
      \end{enumerate}
      ...
      \textbf{Steps}:        
      \begin{enumerate}
        \item Player moves adjacent to an Intractable Entity.
        ...
        Intractable Entity and updates the Entity's state accordingly.
        \item Player moves away from the Intractable Entity.
  ...
  \begin{section}{Combat}
    \begin{subsection}{Engage Combat}
      ...
      \begin{enumerate}
        ...
        \item The use case Combat Turn is now performed.
      \end{enumerate}
    \end{subsection}
...
include the player, monster, communications, game state and map.
  ...
  \begin{section}{Player}
    \begin{subsection}{Player Movement}
      ...
      \textbf{Related Use Cases}:
      All over world interactions are extended by this use case. Combat and Level Transfer are 
      examples.
    ...
    \begin{subsection}{Level Transfer}
      ...
      \textbf{Summary}:
      The player has found the portal tile and would like to transfer to or from each level
